...
double *subgraph_knn_max_distances_evaluate (struct subgraph * sg, int kmax);           /* creates arcs for each node (adjacency relation) and returns
...
double  subgraph_k_max_normalized_cut       (struct subgraph * sg);                     /* normalized cut computed only for sg->bestk neighbors         */
