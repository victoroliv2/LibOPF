...
int
SgAncestor (int *dad, int *cmap, int rq)
{
  int r, ro;
...
int
SgRepresentative (int *cmap, int p)
{
  if (cmap[p] == p)
    return (p);
  else
...
int *
SgAreaOpen (Subgraph * g, int thres)
{
  SgCTree *ctree = NULL;
  int i, p;
  int *fval;
...
int
SgAreaLevel (SgCTree * ctree, int *level, int i, int thres)
{
...
int
SgVolumeLevel (SgCTree * ctree, int *level, int i, int thres, int cumvol)
{
  int dad, vol = cumvol;
  ...
  if (dad != NIL)
    vol = cumvol +
      abs (ctree->node[i].level -
           ctree->node[dad].level) * ctree->node[i].size;
...
int *
SgVolumeOpen (Subgraph * g, int thres)
{
  SgCTree *ctree = NULL;
  int i, p;
  int *fval = NULL;
